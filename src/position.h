...
public:
    ...
    Position &set(const Variant *v, const std::string &fenStr, bool isChess960, StateInfo *si,
                  Thread *th, bool sfen = false);
    Position &set(const std::string &code, Color c, StateInfo *si);
    std::string fen(bool sfen = false, bool showPromoted = false, int countStarted = 0,
                    std::string holdings = "-") const;
    ...
    if (enclosing_drop()) {
        ...
        if (var->enclosingDropStart & ~pieces())
            ...
            if (enclosing_drop() == REVERSI) {
                ...
                b &= shift<NORTH>(theirs) | shift<SOUTH>(theirs) | shift<NORTH_EAST>(theirs) |
                     shift<SOUTH_WEST>(theirs) | shift<EAST>(theirs) | shift<WEST>(theirs) |
                     shift<SOUTH_EAST>(theirs) | shift<NORTH_WEST>(theirs);
                Bitboard b2 = b;
                while (b2) {
                    Square s = pop_lsb(b2);
                    if (!(attacks_bb(c, QUEEN, s, board_bb() & ~pieces(~c)) &
                          ~PseudoAttacks[c][KING][s] & pieces(c)))
                        b ^= s;
                }
            } else {
                assert(enclosing_drop() == ATAXX);
                Bitboard ours = pieces(c);
                b &= shift<NORTH>(ours) | shift<SOUTH>(ours) | shift<NORTH_EAST>(ours) |
                     shift<SOUTH_WEST>(ours) | shift<EAST>(ours) | shift<WEST>(ours) |
...
inline Value Position::stalemate_value(int ply) const
{
    ...
    if (var->stalematePieceCount) {
        ...
        return c == 0
                   ? VALUE_DRAW
                   : convert_mate_value(c < 0 ? var->stalemateValue : -var->stalemateValue, ply);
    }
...
inline Value Position::checkmate_value(int ply) const
{
    ...
    if (var->shogiPawnDropMateIllegal && !(checkers() & ~pieces(SHOGI_PAWN)) &&
        !st->capturedPiece && st->pliesFromNull > 0 &&
...
inline bool Position::extinction_single_piece() const
{
    ...
    return var->extinctionValue == -VALUE_MATE &&
           std::any_of(var->extinctionPieceTypes.begin(), var->extinctionPieceTypes.end(),
...
{
    return pieces(c) & (pieces(QUEEN) | pieces(AIWOK) | pieces(ARCHBISHOP) | pieces(CHANCELLOR) |
                        pieces(AMAZON));
}
...
inline Bitboard Position::attacks_from(Color c, PieceType pt, Square s) const
{
    ...
    // Janggi palace moves
    if (diagonal_lines() & s) {
        PieceType diagType = movePt == WAZIR     ? FERS
                             : movePt == SOLDIER ? PAWN
                             : movePt == ROOK    ? BISHOP
                                                 : NO_PIECE_TYPE;
        if (diagType)
            ...
            b |= rider_attacks_bb<RIDER_CANNON_DIAG>(s, pieces()) &
                 rider_attacks_bb<RIDER_CANNON_DIAG>(s, pieces() ^ pieces(pt)) & ~pieces(pt) &
                 diagonal_lines();
    }
...
{
    return countStarted == 0 || (count<ALL_PIECES>(WHITE) <= 1 || count<ALL_PIECES>(BLACK) <= 1)
               ? st->countingPly
           : countStarted < 0
               ? 0
               : std::min(st->countingPly, std::max(1 + gamePly - countStarted, 0));
}
...
inline bool Position::capture_or_promotion(Move m) const
{
    ...
    return type_of(m) == PROMOTION || type_of(m) == EN_PASSANT ||
           (type_of(m) != CASTLING && !empty(to_sq(m)));
}
...
inline bool Position::allow_virtual_drop(Color c, PieceType pt) const
{
    ...
    return pt != KING && (count_in_hand(c, PAWN) >= -(pt == PAWN) &&
                          ...
                          count_in_hand(c, ROOK) >= 0 && count_in_hand(c, QUEEN) >= 0);
}

inline Value Position::material_counting_result() const
{
    auto weigth_count = [this](PieceType pt, int v) {
        return v * (count(WHITE, pt) - count(BLACK, pt));
    };
    int materialCount;
    Value result;
    switch (var->materialCounting) {
    case JANGGI_MATERIAL:
        materialCount = weigth_count(ROOK, 13) + weigth_count(JANGGI_CANNON, 7) +
                        weigth_count(HORSE, 5) + weigth_count(JANGGI_ELEPHANT, 3) +
                        weigth_count(WAZIR, 3) + weigth_count(SOLDIER, 2) - 1;
