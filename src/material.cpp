...
bool is_KFsPsK(const Position &pos, Color us)
{
    return pos.promotion_piece_types().size() == 1 &&
           ...
           !more_than_one(pos.pieces(~us)) && (pos.count<FERS>(us) || pos.count<PAWN>(us)) &&
           !(pos.count<ALL_PIECES>(us) - pos.count<FERS>(us) - pos.count<PAWN>(us) -
             pos.count<KING>(us));
}
...
template <Color Us> Score imbalance(const Position &pos, const int pieceCount[][PIECE_TYPE_NB])
{
    ...
    if (pos.must_capture())
        bonus += (make_score(mg_value(QuadraticOurs[PAWN][PAWN]), 0) * pieceCount[Us][PAWN] +
                  ...
                  QuadraticOurs[QUEEN][PAWN] * pieceCount[Us][QUEEN]) *
                     pieceCount[Us][PAWN] +
                 make_score(mg_value(QuadraticOurs[KNIGHT][KNIGHT]),
                            -eg_value(QuadraticOurs[KNIGHT][KNIGHT])) *
                     pieceCount[Us][KNIGHT] * pieceCount[Us][KNIGHT];
    else if (pos.blast_on_capture())
        bonus -= make_score(mg_value(QuadraticOurs[KNIGHT][PAWN]) * pieceCount[Us][KNIGHT] *
                                pieceCount[Us][PAWN] / 2,
                            0);
...
Entry *probe(const Position &pos)
{
    ...
    if (pos.captures_to_hand() || pos.two_boards()) {
        ...
        int countAll =
            pos.count_with_hand(WHITE, ALL_PIECES) + pos.count_with_hand(BLACK, ALL_PIECES);
        e->materialDensity = (npm + npm2 + pos.count<PAWN>() * PawnValueMg) * countAll /
                             (pos.files() * pos.ranks());
    } else
        e->gamePhase =
    ...
    if (pos.endgame_eval()) {
        ...
        if (!pos.count<PAWN>(WHITE) && npm_w - npm_b <= BishopValueMg)
            e->factor[WHITE] = uint8_t(
                npm_w < RookValueMg && pos.count<ALL_PIECES>(WHITE) <= 2      ? SCALE_FACTOR_DRAW
                : npm_b <= BishopValueMg && pos.count<ALL_PIECES>(WHITE) <= 3 ? 4
                                                                              : 14);

        if (!pos.count<PAWN>(BLACK) && npm_b - npm_w <= BishopValueMg)
            e->factor[BLACK] = uint8_t(
                npm_b < RookValueMg && pos.count<ALL_PIECES>(BLACK) <= 2      ? SCALE_FACTOR_DRAW
                : npm_w <= BishopValueMg && pos.count<ALL_PIECES>(BLACK) <= 3 ? 4
                                                                              : 14);
    }
