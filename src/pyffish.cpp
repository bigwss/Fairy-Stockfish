...
void buildPosition(Position &pos, StateListPtr &states, const char *variant, const char *fen,
                   PyObject *moveList, const bool chess960)
{
    ...
    for (int i = 0; i < numMoves; i++) {
        PyObject *MoveStr =
            PyUnicode_AsEncodedString(PyList_GetItem(moveList, i), "UTF-8", "strict");
...
extern "C" PyObject *pyffish_twoBoards(PyObject *self, PyObject *args)
{
    ...
    return Py_BuildValue("O", variants.find(std::string(variant))->second->twoBoards ? Py_True
                                                                                     : Py_False);
}
...
extern "C" PyObject *pyffish_getSAN(PyObject *self, PyObject *args)
{
    ...
    return Py_BuildValue("s",
                         SAN::move_to_san(pos, UCI::to_move(pos, moveStr), notation).c_str());
}
...
extern "C" PyObject *pyffish_hasInsufficientMaterial(PyObject *self, PyObject *args)
{
    ...
    return Py_BuildValue("(OO)", wInsufficient ? Py_True : Py_False,
                         bInsufficient ? Py_True : Py_False);
}
...
extern "C" PyObject *pyffish_validateFen(PyObject *self, PyObject *args)
{
    ...
    return Py_BuildValue("i", FEN::validate_fen(std::string(fen),
                                                variants.find(std::string(variant))->second,
                                                chess960));
}
...
static PyMethodDef PyFFishMethods[] = {
    ...
    { "two_boards", (PyCFunction)pyffish_twoBoards, METH_VARARGS,
      "Checks whether the variant is played on two boards." },
    { "get_san", (PyCFunction)pyffish_getSAN, METH_VARARGS,
      "Get SAN move from given FEN and UCI move." },
    { "get_san_moves", (PyCFunction)pyffish_getSANmoves, METH_VARARGS,
      "Get SAN movelist from given FEN and UCI movelist." },
    ...
    { "game_result", (PyCFunction)pyffish_gameResult, METH_VARARGS,
      "Get result from given FEN, considering variant end, checkmate, and stalemate." },
    { "is_immediate_game_end", (PyCFunction)pyffish_isImmediateGameEnd, METH_VARARGS,
      "Get result from given FEN if variant rules ends the game." },
    { "is_optional_game_end", (PyCFunction)pyffish_isOptionalGameEnd, METH_VARARGS,
      "Get result from given FEN it rules enable game end by player." },
...
static PyModuleDef pyffishmodule = {
    PyModuleDef_HEAD_INIT, "pyffish", "Fairy-Stockfish extension module.", -1, PyFFishMethods,
};
...
PyMODINIT_FUNC PyInit_pyffish()
{
    ...
    PyModule_AddObject(module, "NOTATION_SHOGI_HODGES_NUMBER",
                       PyLong_FromLong(NOTATION_SHOGI_HODGES_NUMBER));
