...
template <MoveType T>
ExtMove *make_move_and_gating(const Position &pos, ExtMove *moveList, Color us, Square from,
                              Square to)
{
    ...
    if (pos.arrow_gating()) {
        for (PieceType pt_gating : pos.piece_types())
            if (pos.count_in_hand(us, pt_gating) > 0) {
                Bitboard b = pos.drop_region(us, pt_gating) &
                             moves_bb(us, type_of(pos.piece_on(from)), to, pos.pieces() ^ from) &
                             ~(pos.pieces() ^ from);
                while (b)
...
ExtMove *generate_drops(const Position &pos, ExtMove *moveList, PieceType pt, Bitboard b)
{
    ...
    if (pos.count_in_hand(Us, pt) > 0 ||
        (Type != NON_EVASIONS && pos.two_boards() && pos.allow_virtual_drop(Us, pt))) {
...
ExtMove *generate_pawn_moves(const Position &pos, ExtMove *moveList, Bitboard target)
{
    ...
    Bitboard TRank8BB = pos.mandatory_pawn_promotion()
                            ? rank_bb(relative_rank(Us, pos.promotion_rank(), pos.max_rank()))
    ...
    Bitboard TRank3BB =
        forward_ranks_bb(Us, relative_rank(Us, pos.double_step_rank_min(), pos.max_rank())) &
        ~shift<Up>(
            forward_ranks_bb(Us, relative_rank(Us, pos.double_step_rank_max(), pos.max_rank())));

    const Bitboard emptySquares =
        Type == QUIETS || Type == QUIET_CHECKS ? target : ~pos.pieces() & pos.board_bb();
    const Bitboard enemies =
        Type == EVASIONS
            ? (pos.checkers() & pos.non_sliding_riders() ? pos.pieces(Them) : pos.checkers())
        : Type == CAPTURES ? target
                           : pos.pieces(Them);

    Bitboard pawnsOn7 = pos.pieces(Us, PAWN) & TRank7BB;
    Bitboard pawnsNotOn7 =
        pos.pieces(Us, PAWN) & (pos.mandatory_pawn_promotion() ? ~TRank7BB : AllSquares);

    // Single and double pawn pushes, no promotions
    if (Type != CAPTURES) {
        Bitboard b1 = shift<Up>(pawnsNotOn7) & emptySquares;
        Bitboard b2 =
            pos.double_step_enabled() ? shift<Up>(b1 & TRank3BB) & emptySquares : Bitboard(0);
    ...
    if (pos.sittuyin_promotion() &&
        ...
        if (pos.count<PAWN>(Us) > 1)
            pawns &=
                (PseudoAttacks[Us][BISHOP]
                              [make_square(FILE_A, relative_rank(Us, RANK_1, pos.max_rank()))] |
                 PseudoAttacks[Us][BISHOP][make_square(
                     pos.max_file(), relative_rank(Us, RANK_1, pos.max_rank()))]) &
                forward_ranks_bb(
                    Us, relative_rank(Us, Rank((pos.max_rank() - 1) / 2), pos.max_rank()));
        while (pawns) {
    ...
    if (Type == CAPTURES || Type == EVASIONS || Type == NON_EVASIONS) {
        ...
        if (pos.ep_square() != SQ_NONE) {
            assert(relative_rank(Them, rank_of(pos.ep_square()), pos.max_rank()) <=
                   Rank(pos.double_step_rank_max() + 1));
...
ExtMove *generate_moves(const Position &pos, ExtMove *moveList, PieceType Pt, Bitboard target)
{
    ...
    while (bb) {
        ...
        Bitboard b2 = promPt && (!pos.promotion_limit(promPt) ||
                                 pos.promotion_limit(promPt) > pos.count(Us, promPt))
                          ? b1
                          : Bitboard(0);
        ...
        if (b2 | b3) {
            ...
            if (pos.mandatory_piece_promotion())
                b1 &= (promotion_zone & from ? Bitboard(0) : ~promotion_zone) |
                      (pos.piece_promotion_on_capture() ? ~pos.pieces() : Bitboard(0));
...
template <Color Us, GenType Type> ExtMove *generate_all(const Position &pos, ExtMove *moveList)
{
    ...
    if (Type != EVASIONS || !more_than_one(pos.checkers() & ~pos.non_sliding_riders())) {
        ...
        if (pos.piece_drops() && Type != CAPTURES &&
            (pos.count_in_hand(Us, ALL_PIECES) > 0 || pos.two_boards()))
            for (PieceType pt : pos.piece_types())
        ...
        if (!pos.count<KING>(Us) && Type != CAPTURES && pos.can_castle(Us & ANY_CASTLING)) {
            ...
            for (CastlingRights cr : { Us & KING_SIDE, Us & QUEEN_SIDE })
                if (!pos.castling_impeded(cr) && pos.can_castle(cr))
                    moveList = make_move_and_gating<CASTLING>(pos, moveList, Us, from,
        ...
        if (pos.cambodian_moves() && pos.gates(Us)) {
            if (Type != CAPTURES && Type != EVASIONS && (pos.pieces(Us, KING) & pos.gates(Us))) {
                ...
                Bitboard b = PseudoAttacks[WHITE][KNIGHT][from] &
                             rank_bb(rank_of(from + (Us == WHITE ? NORTH : SOUTH))) & target &
                             ~pos.pieces();
                while (b)
    ...
    if (pos.count<KING>(Us) && (!Checks || pos.blockers_for_king(~Us) & ksq)) {
        ...
        if ((Type == QUIETS || Type == NON_EVASIONS) && pos.can_castle(Us & ANY_CASTLING))
            for (CastlingRights cr : { Us & KING_SIDE, Us & QUEEN_SIDE })
                if (!pos.castling_impeded(cr) && pos.can_castle(cr))
                    moveList = make_move_and_gating<CASTLING>(pos, moveList, Us, ksq,
                                                              pos.castling_rook_square(cr));
    }
...
/// <QUIET_CHECKS> Generates all pseudo-legal non-captures giving check, except castling and
/// promotions <NON_EVASIONS> Generates all pseudo-legal captures and non-captures
///
