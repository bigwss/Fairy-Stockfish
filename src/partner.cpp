...
template <PartnerType p> void PartnerHandler::ptell(const std::string &message)
{
...
void PartnerHandler::parse_ptell(std::istringstream &is, const Position &pos)
{
    ...
    if (token == "partner") {
        ...
        if (!(is >> token)) {
            ptell<HUMAN>("I listen to the commands help, sit, go, move, fast, slow, dead, x, "
                         "time, and otim.");
            ...
            ptell<HUMAN>("'time' together with your time in centiseconds allows me to consider "
                         "your time.");
            ptell<HUMAN>("E.g., 'time 1000' for 10 seconds.");
        } else if (token == "otim")
            ptell<HUMAN>("'otim' together with your opponent's time in centiseconds allows me to "
                         "consider his time.");
