...
#endif
#ifdef LARGEBOARDS
constexpr Bitboard DarkSquares =
    (Bitboard(0xAAA555AAA555AAULL) << 64) ^ Bitboard(0xA555AAA555AAA555ULL);
#else
...
constexpr Bitboard FileABB =
    (Bitboard(0x00100100100100ULL) << 64) ^ Bitboard(0x1001001001001001ULL);
#else
...
constexpr Bitboard make_bitboard()
{
    return 0;
}

template <typename... Squares> constexpr Bitboard make_bitboard(Square s, Squares... squares)
{
...
{
    return c == WHITE
               ? (AllSquares ^ Rank1BB) << FILE_NB * relative_rank(WHITE, s, RANK_MAX)
...
{
    return forward_ranks_bb(c, relative_rank(c, r, maxRank)) |
           rank_bb(relative_rank(c, r, maxRank));
}
...
inline int edge_distance(File f, File maxFile = FILE_H)
{
    return std::min(f, File(maxFile - f));
}
inline int edge_distance(Rank r, Rank maxRank = RANK_8)
{
    return std::min(r, Rank(maxRank - r));
}

template <RiderType R> inline Bitboard rider_attacks_bb(Square s, Bitboard occupied)
{
...
template <PieceType Pt> inline Bitboard attacks_bb(Square s, Bitboard occupied)
{
    ...
    case ROOK:
        return rider_attacks_bb<RIDER_ROOK_H>(s, occupied) |
