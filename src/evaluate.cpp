...
[[maybe_unused]] const unsigned char *const gEmbeddedNNUE2End = &gEmbeddedNNUE2Data[1];
...
void NNUE::init()
{
    ...
    for (string directory : dirs)
        if (eval_file_loaded != eval_file) {
            ...
            if (directory == "<internal>" &&
                (eval_file == EvalFileDefaultName || eval_file == EvalFile2DefaultName)) {
                ...
                MemoryBuffer buffer(const_cast<char *>(reinterpret_cast<const char *>(
                                        first ? gEmbeddedNNUEData : gEmbeddedNNUE2Data)),
...
void NNUE::verify()
{
    ...
    if (useNNUE && eval_file.find(eval_file_loaded) == string::npos) {
        ...
        string msg1 = "If the UCI option \"Use NNUE\" is set to true, network evaluation "
        ...
        string msg3 = "The UCI option EvalFile might need to specify the full path, including "
    ...
    if (useNNUE)
        sync_cout << "info string NNUE evaluation using " << eval_file_loaded << " enabled"
                  << sync_endl;
    else
...
constexpr int SafeCheck[][2] = { {},           { 600, 600 },   { 803, 1292 },
...
constexpr Score MobilityBonus[][4 * RANK_NB] = {
    ...
    { S(-60, -82), S(-24, -15), S(0, 17), S(3, 43), S(4, 72), S(14, 100),  // Rook
      S(20, 102), S(30, 122), S(41, 133), S(41, 139), S(41, 153), S(45, 160), S(57, 165),
      S(58, 170), S(67, 175) },
...
constexpr Score BishopPawns[int(FILE_NB) / 2] = { S(3, 8), S(3, 9), S(2, 8), S(3, 8) };
...
constexpr Score PassedRank[RANK_NB] = { S(0, 0),   S(7, 27),    S(16, 32),  S(17, 40),
                                        S(64, 71), S(170, 174), S(278, 262) };
...
constexpr Score ThreatByMinor[PIECE_TYPE_NB] = { S(0, 0),   S(5, 32),   S(55, 41),
...
template <Tracing T> template <Color Us> void Evaluation<T>::initialize()
{
    ...
    Bitboard LowRanks = rank_bb(relative_rank(Us, RANK_2, pos.max_rank())) |
    ...
    if (pos.must_capture())
        ...
        mobilityArea[Us] = ~(
            ...
            shift<EAST>(pos.promoted_soldiers(Them)) | shift<WEST>(pos.promoted_soldiers(Them)));
    ...
    attackedBy[Us][ALL_PIECES] =
        attackedBy[Us][KING] | attackedBy[Us][PAWN] | attackedBy[Us][SHOGI_PAWN];
    attackedBy2[Us] = (attackedBy[Us][KING] & attackedBy[Us][PAWN]) |
                      (attackedBy[Us][KING] & attackedBy[Us][SHOGI_PAWN]) |
                      (attackedBy[Us][PAWN] & attackedBy[Us][SHOGI_PAWN]) | dblAttackByPawn;
    ...
    kingAttackersCount[Them] = popcount(
        kingRing[Us] & (pe->pawn_attacks(Them) | shift<Down>(pos.pieces(Them, SHOGI_PAWN))));
...
template <Tracing T> template <Color Us> Score Evaluation<T>::pieces(PieceType Pt)
{
    ...
    while (b1) {
        ...
        if (pos.promoted_piece_type(Pt) != NO_PIECE_TYPE) {
            ...
            if (zone & (b | s))
                score +=
                    make_score(PieceValue[MG][pos.promoted_piece_type(Pt)] - PieceValue[MG][Pt],
                               PieceValue[EG][pos.promoted_piece_type(Pt)] - PieceValue[EG][Pt]) /
                    (zone & s && b ? 6 : 12);
            ...
            score -= make_score(PieceValue[MG][Pt] - PieceValue[MG][pos.unpromoted_piece_on(s)],
                                PieceValue[EG][Pt] - PieceValue[EG][pos.unpromoted_piece_on(s)]) /
                     4;
        ...
        if ((pos.captures_to_hand() || pos.two_boards()) && pos.count<KING>(Them) &&
            pos.count<KING>(Us)) {
            if (!(b & (kingRing[Us] | kingRing[Them])))
                score -= KingProximity * distance(s, pos.square<KING>(Us)) *
                         distance(s, pos.square<KING>(Them));
        }
        ...
        if (Pt == BISHOP || Pt == KNIGHT) {
            ...
            if (Pt == BISHOP) {
                ...
                score -= BishopPawns[edge_distance(file_of(s), pos.max_file())] *
                         pos.pawns_on_same_color_squares(Us, s) *
                         (!(attackedBy[Us][PAWN] & s) + popcount(blocked & CenterFiles));

                // Penalty for all enemy pawns x-rayed
                score -=
                    BishopXRayPawns * popcount(attacks_bb<BISHOP>(s) & pos.pieces(Them, PAWN));
...
template <Tracing T> template <Color Us> Score Evaluation<T>::hand(PieceType pt)
{
    ...
    if (pos.count_in_hand(Us, pt) > 0 && pt != KING) {
        Bitboard b = pos.drop_region(Us, pt) & ~pos.pieces() &
                     (~attackedBy2[Them] | attackedBy[Us][ALL_PIECES]);
        if ((b & kingRing[Them]) && pt != SHOGI_PAWN) {
            kingAttackersCountInHand[Us] += pos.count_in_hand(Us, pt);
            kingAttackersWeightInHand[Us] +=
                KingAttackWeights[std::min(pt, FAIRY_PIECES)] * pos.count_in_hand(Us, pt);
            kingAttacksCount[Us] += popcount(b & attackedBy[Them][KING]);
        }
        Bitboard theirHalf =
            pos.board_bb() &
            ~forward_ranks_bb(
                Them, relative_rank(Them, Rank((pos.max_rank() - 1) / 2), pos.max_rank()));
        ...
        if (pos.promoted_piece_type(pt) != NO_PIECE_TYPE && pos.drop_promoted())
            score +=
                make_score(
                    std::max(PieceValue[MG][pos.promoted_piece_type(pt)] - PieceValue[MG][pt],
                             VALUE_ZERO),
                    std::max(PieceValue[EG][pos.promoted_piece_type(pt)] - PieceValue[EG][pt],
                             VALUE_ZERO)) /
                4 * pos.count_in_hand(Us, pt);
        ...
        if (pos.seirawan_gating() && !pos.piece_drops() &&
            pos.count_in_hand(Us, ALL_PIECES) > popcount(pos.gates(Us)))
            score -= make_score(200, 900) / pos.count_in_hand(Us, ALL_PIECES) *
                     (pos.count_in_hand(Us, ALL_PIECES) - popcount(pos.gates(Us)));

        // Redundant pieces that can not be doubled per file (e.g., shogi pawns)
        if (pt == pos.drop_no_doubled())
            score -= make_score(50, 20) *
                     std::max(pos.count_with_hand(Us, pt) - pos.max_file() - 1, 0);
    }
...
template <Tracing T> template <Color Us> Score Evaluation<T>::king() const
{
    ...
    constexpr Color Them = ~Us;
    Rank r = relative_rank(Us, std::min(Rank((pos.max_rank() - 1) / 2 + 1), pos.max_rank()),
                           pos.max_rank());
    ...
    std::function<Bitboard(Color, PieceType)> get_attacks = [this](Color c, PieceType pt) {
        return attackedBy[c][pt] | (pos.piece_drops() && pos.count_in_hand(c, pt) > 0
                                        ? pos.drop_region(c, pt) & ~pos.pieces()
                                        : Bitboard(0));
    };
    for (PieceType pt : pos.piece_types()) {
        ...
        case QUEEN:
            ...
            queenChecks = (b1 | b2) & get_attacks(Them, QUEEN) & pos.board_bb() & safe &
                          ~attackedBy[Us][QUEEN] & ~(b1 & attackedBy[Them][ROOK]);
        ...
        case BISHOP:
        case KNIGHT:
            knightChecks = attacks_bb(Us, pt, ksq, pos.pieces() ^ pos.pieces(Us, QUEEN)) &
                           get_attacks(Them, pt) & pos.board_bb();
        ...
        case PAWN:
            if (pos.piece_drops() && pos.count_in_hand(Them, pt) > 0) {
                pawnChecks =
                    attacks_bb(Us, pt, ksq, pos.pieces()) & ~pos.pieces() & pos.board_bb();
                if (pawnChecks & safe)
        ...
        case SHOGI_PAWN:
            if (pos.promoted_piece_type(pt)) {
                otherChecks = attacks_bb(Us, pos.promoted_piece_type(pt), ksq, pos.pieces()) &
                              attackedBy[Them][pt] &
                              zone_bb(Them, pos.promotion_rank(), pos.max_rank()) &
                              pos.board_bb();
                if (otherChecks & safe)
        ...
        default:
            otherChecks =
                attacks_bb(Us, pt, ksq, pos.pieces()) & get_attacks(Them, pt) & pos.board_bb();
            if (otherChecks & safe)
                kingDanger += SafeCheck[FAIRY_PIECES][more_than_one(otherChecks & safe)];
            else
                unsafeChecks |= otherChecks;
    ...
    if (pos.two_boards() && pos.piece_drops()) {
        for (PieceType pt : pos.piece_types())
            if (pos.count_in_hand(Them, pt) <= 0 &&
                (attacks_bb(Us, pt, ksq, pos.pieces()) & safe & pos.drop_region(Them, pt) &
                 ~pos.pieces())) {
    ...
    Square s = file_of(ksq) == FILE_A           ? ksq + EAST
               : file_of(ksq) == pos.max_file() ? ksq + WEST
                                                : ksq;
    Bitboard kingFlank =
        pos.max_file() == FILE_H ? KingFlank[file_of(ksq)] : file_bb(s) | adjacent_files_bb(s);
    ...
    kingDanger += kingAttackersCount[Them] * kingAttackersWeight[Them] +
                  ...
                  183 * popcount(kingRing[Us] & (weak | ~pos.board_bb(Us, KING))) *
                      (1 + pos.captures_to_hand() + pos.check_counting()) +
                  148 * popcount(unsafeChecks) * (1 + pos.check_counting()) +
                  98 * popcount(pos.blockers_for_king(Us)) +
                  69 * kingAttacksCount[Them] *
                      (2 + 8 * pos.check_counting() + pos.captures_to_hand()) / 2 +
                  3 * kingFlankAttack * kingFlankAttack / 8 +
                  mg_value(mobility[Them] - mobility[Us]) * int(!pos.captures_to_hand()) -
                  873 * !(pos.major_pieces(Them) || pos.captures_to_hand()) * 2 /
                      (2 + 2 * pos.check_counting() + 2 * pos.two_boards() + 2 * pos.makpong() +
    ...
    score -=
        FlankAttacks * kingFlankAttack * (1 + 5 * pos.captures_to_hand() + pos.check_counting());
...
template <Tracing T> template <Color Us> Score Evaluation<T>::threats() const
{
    ...
    if (pos.must_capture()) {
        ...
        if (captures)
            score -= make_score(2000, 2000) /
                     (1 + popcount(captures & attackedBy[Them][ALL_PIECES] & ~attackedBy2[Us]));
        ...
        score +=
            make_score(200, 200) * popcount(attackedBy[Them][ALL_PIECES] & moves & ~pos.pieces());
        score += make_score(200, 220) * popcount(attackedBy[Them][ALL_PIECES] & moves &
                                                 ~pos.pieces() & ~attackedBy2[Us]);
    }
    ...
    if (pos.extinction_value() == -VALUE_MATE) {
        ...
        for (PieceType pt : pos.extinction_piece_types()) {
            ...
            if (pos.blast_on_capture()) {
                int evasions =
                    popcount(((attackedBy[Them][pt] & ~pos.pieces(Them)) | pos.pieces(Them, pt)) &
                             ~attackedBy[Us][ALL_PIECES]) *
                    denom;
                int attacks = popcount((attackedBy[Them][pt] | pos.pieces(Them, pt)) &
                                       attackedBy[Us][ALL_PIECES]);
                int explosions = 0;
                ...
                while (bExtBlast) {
                    ...
                    if (((attacks_bb<KING>(s) | s) & pos.pieces(Them, pt)) &&
                        !(attacks_bb<KING>(s) & pos.pieces(Us, pt)))
                        explosions++;
                }
                ...
                score += make_score(1000, 1000) / (denom * denom) *
                         popcount(bExt & pos.pieces(Them, pt));
        }
    }
    ...
    stronglyProtected =
        (attackedBy[Them][PAWN] | attackedBy[Them][SHOGI_PAWN] | attackedBy[Them][SOLDIER]) |
    ...
    b = (pawn_attacks_bb<Us>(b) | shift<Up>(shift<Up>(pos.pieces(Us, SHOGI_PAWN, SOLDIER)))) &
        nonPawnEnemies;
...
template <Tracing T> template <Color Us> Score Evaluation<T>::passed() const
{
    ...
    auto king_proximity = [&](Color c, Square s) {
        return pos.extinction_value() == VALUE_MATE ? 0
               : pos.count<KING>(c) ? std::min(distance(pos.square<KING>(c), s), 5)
                                    : 5;
    };
    ...
    while (b) {
        ...
        int r = std::max(
            RANK_8 - std::max(pos.promotion_rank() - relative_rank(Us, s, pos.max_rank()), 0), 0);
        ...
        if (r > RANK_3) {
            ...
            bonus += make_score(
                0,
                (king_proximity(Them, blockSq) * 19 / 4 - king_proximity(Us, blockSq) * 2) * w);
            ...
            if (pos.empty(blockSq)) {
                ...
                // If there are no enemy pieces or attacks on passed pawn span, assign a big
                // bonus. Or if there is some, but they are all attacked by our pawns, assign a
                // bit smaller bonus. Otherwise assign a smaller bonus if the path to queen is not
                // attacked and even smaller bonus if it is attacked but block square is not.
    ...
    if (pt != NO_PIECE_TYPE) {
        ...
        while (b) {
            Square s = pop_lsb(b);
            if ((pos.pieces(Them, SHOGI_PAWN) & forward_file_bb(Us, s)) ||
                relative_rank(Us, s, pos.max_rank()) == pos.max_rank())
                continue;
...
template <Tracing T> template <Color Us> Score Evaluation<T>::space() const
{
    ...
    if (pawnsOnly) {
        safe = pos.board_bb() & ((attackedBy2[Us] & ~attackedBy2[Them]) |
                                 (attackedBy[Us][PAWN] & ~pos.pieces(Us, PAWN)));
        behind = 0;
    }
...
template <Tracing T> template <Color Us> Score Evaluation<T>::variant() const
{
    ...
    if (pos.capture_the_flag(Us)) {
        ...
        for (int dist = 0; (ctfPieces || onHold || onHold2) && (ctfTargets & ~processed);
             dist++) {
            ...
            if (pt != ALL_PIECES) {
                ...
                if (pos.count(Them, pt) >= pos.extinction_opponent_piece_count() ||
                    ...
                    score += make_score(1000000 / (500 + PieceValue[MG][pt]),
                                        1000000 / (500 + PieceValue[EG][pt])) /
                             (denom * denom) * (pos.extinction_value() / VALUE_MATE);
            } else if (pos.extinction_value() == VALUE_MATE) {
                // Losing chess variant bonus
                score += make_score(pos.non_pawn_material(Us), pos.non_pawn_material(Us)) /
                         std::max(pos.count<ALL_PIECES>(Us), 1);
                ...
                for (File f = FILE_A; f <= pos.max_file(); ++f) {
                    l = m;
                    m = r;
                    r = popcount(pos.pieces(Us, PAWN) & shift<EAST>(file_bb(f)));
                ...
                int dist = 8;
                Bitboard breakthroughs =
                    attackedBy[Us][ALL_PIECES] &
                    rank_bb(relative_rank(Us, pos.max_rank(), pos.max_rank()));
                if (breakthroughs)
                    dist = attackedBy[Us][QUEEN] & breakthroughs ? 0 : 1;
                else
                    for (File f = FILE_A; f <= pos.max_file(); ++f)
                        dist = std::min(dist, popcount(pos.pieces(PAWN) & file_bb(f)));
                score += make_score(70, 70) * pos.count<PAWN>(Them) / (1 + dist * dist) /
                         (pos.pieces(Us, QUEEN) ? 2 : 4);
            }
    }
    ...
    if (pos.flip_enclosed_pieces()) {
        ...
        if (pos.flip_enclosed_pieces() == REVERSI) {
            Bitboard edges =
                (FileABB | file_bb(pos.max_file()) | Rank1BB | rank_bb(pos.max_rank())) &
                pos.board_bb();
            Bitboard edgePieces = pos.pieces(Us) & edges;
            while (edgePieces) {
                Bitboard connectedEdge =
                    attacks_bb(Us, ROOK, pop_lsb(edgePieces), ~(pos.pieces(Us) & edges)) & edges;
        ...
        while (drops) {
            ...
            if (pos.flip_enclosed_pieces() == REVERSI) {
                Bitboard b = attacks_bb(Them, QUEEN, s, ~pos.pieces(Us)) &
                             ~PseudoAttacks[Them][KING][s] & pos.pieces(Them);
                while (b)
...
template <Tracing T> Value Evaluation<T>::winnable(Score score) const
{
    ...
    if (pos.extinction_value() == VALUE_NONE && !pos.captures_to_hand() && !pos.connect_n() &&
        ...
        pawnsOnBothFlanks = (pos.pieces(PAWN) & QueenSide) && (pos.pieces(PAWN) & KingSide);

        bool almostUnwinnable =
            outflanking < 0 && pos.stalemate_value() == VALUE_DRAW && !pawnsOnBothFlanks;

        bool infiltration =
            (pos.count<KING>(WHITE) && rank_of(pos.square<KING>(WHITE)) > RANK_4) ||
            (pos.count<KING>(BLACK) && rank_of(pos.square<KING>(BLACK)) < RANK_5);

        // Compute the initiative bonus for the attacking side
        complexity = 9 * pe->passed_count() + 12 * pos.count<PAWN>() + 15 * pos.count<SOLDIER>() +
                     9 * outflanking + 21 * pawnsOnBothFlanks + 24 * infiltration +
                     51 * !pos.non_pawn_material() - 43 * almostUnwinnable - 110;
    }
...
template <Tracing T> Value Evaluation<T>::value()
{
    ...
    score += (mobility[WHITE] - mobility[BLACK]) *
             (1 + pos.captures_to_hand() + pos.must_capture() + pos.check_counting());
    ...
    score += threats<WHITE>() - threats<BLACK>() + space<WHITE>() - space<BLACK>();
...
Value fix_FRC(const Position &pos)
{
    ...
    constexpr Bitboard Corners = Bitboard(1ULL) << SQ_A1 | Bitboard(1ULL) << SQ_H1 |
                                 Bitboard(1ULL) << SQ_A8 | Bitboard(1ULL) << SQ_H8;
...
Value Eval::evaluate(const Position &pos)
{
    ...
    if (!Eval::useNNUE || !pos.nnue_applicable())
        ...
        auto adjusted_NNUE = [&]() {
            int scale = 903 + 32 * pos.count<PAWN>() + 32 * pos.non_pawn_material() / 1024;
...
std::string Eval::trace(Position &pos)
{
    ...
    ss << std::showpoint << std::noshowpos << std::fixed << std::setprecision(2)
       ...
       << "|      Pawns | " << Term(PAWN) << "|    Knights | " << Term(KNIGHT)
       << "|    Bishops | " << Term(BISHOP) << "|      Rooks | " << Term(ROOK)
       << "|     Queens | " << Term(QUEEN) << "|   Mobility | " << Term(MOBILITY)
       << "|King safety | " << Term(KING) << "|    Threats | " << Term(THREAT)
       << "|     Passed | " << Term(PASSED) << "|      Space | " << Term(SPACE)
       << "|    Variant | " << Term(VARIANT) << "|   Winnable | " << Term(WINNABLE)
       << "+------------+-------------+-------------+-------------+\n"
       << "|      Total | " << Term(TOTAL)
       << "+------------+-------------+-------------+-------------+\n";
