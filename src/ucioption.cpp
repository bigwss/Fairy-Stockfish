...
std::set<string> standard_variants = { "normal",   "nocastle",   "fischerandom", "knightmate",
                                       "3check",   "makruk",     "shatranj",     "asean",
                                       "seirawan", "crazyhouse", "bughouse",     "suicide",
                                       "giveaway", "losers",     "atomic",       "capablanca",
                                       "gothic",   "janus",      "caparandom",   "grand",
                                       "shogi",    "xiangqi" };
...
void on_variant_change(const Option &o)
{
    ...
    if (Options["Protocol"] == "xboard") {
        ...
        if (itJanggi != variants.end() &&
            ...
            sync_cout
                << "setup (PH.R.AE..K.C.ph.r.ae..k.c.) 9x10+0_janggi "
        ...
        for (PieceType pt : v->pieceTypes) {
            ...
            if (pt == KING && v->castling)
                suffix += "O" + std::to_string(
                                    (v->castlingKingsideFile - v->castlingQueensideFile) / 2);
            // Drop region
            if (v->pieceDrops) {
                if (pt == PAWN && !v->firstRankPawnDrops)
                    ...
                    suffix += "s";
                suffix += "@" + std::to_string(pt == PAWN && !v->promotionZonePawnDrops
                                                   ? v->promotionRank
                                                   : v->maxRank + 1);
            }
...
void init(OptionsMap &o)
{
    ...
    // Output Format: Output all possible moves.
    o["MultiMoves"] << Option(false);
    ...
    o["EvalFile"] << Option(
        (std::string(EvalFileDefaultName) + UCI::SepChar + EvalFile2DefaultName).c_str(),
        on_eval_file);
#else
            ...
            for (const auto &it : om)
                if (it.second.idx == idx) {
                    ...
                    if (Options["Protocol"] == "ucci" || Options["Protocol"] == "usi") {
                        ...
                        // UCCI skips "name"
                        os << "\noption " << (Options["Protocol"] == "ucci" ? "" : "name ")
                           << name << " type " << o.type;
                    } else
...
Option::Option(const char *v, const std::vector<std::string> &values, OnChange f)
        : type("combo"), min(0), max(0), comboValues(values), on_change(f)
{
    defaultValue = currentValue = v;
}
...
Option &Option::operator=(const string &v)
{
    ...
    if ((type != "button" && v.empty()) || (type == "check" && v != "true" && v != "false") ||
        (type == "combo" &&
