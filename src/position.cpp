...
std::ostream &operator<<(std::ostream &os, const Position &pos)
{
        ...
        if (r == pos.max_rank() || r == RANK_1) {
            ...
            if (pos.piece_drops() || pos.seirawan_gating() || pos.arrow_gating()) {
                ...
                for (PieceType pt = KING; pt >= PAWN; --pt)
                    os << std::string(pos.count_in_hand(c, pt),
                                      pos.piece_to_char()[make_piece(c, pt)]);
                os << "]";
            }
        }
        os << "\n ";
    ...
    os << "\n";
    os << "\nFen: " << pos.fen() << "\nSfen: " << pos.fen(true) << "\nKey: " << std::hex
       << std::uppercase << std::setfill('0') << std::setw(16) << pos.key() << std::setfill(' ')
       << std::dec << "\nCheckers: ";
...
Position &Position::set(const Variant *v, const string &fenStr, bool isChess960, StateInfo *si,
                        Thread *th, bool sfen)
{
    /*
    ...
    */
    ...
    while ((ss >> token) && !isspace(token)) {
        if (isdigit(token)) {
            ...
            if (!is_ok(sq))
                break;
                 ...
                 (idx = piece_to_char_synonyms().find(token)) != string::npos) {
            if (ss.peek() == '~')
                ss >> token;
        ...
        // Promoted shogi pieces
            ...
            ss >> token;
            put_piece(make_piece(color_of(Piece(idx)), promoted_piece_type(type_of(Piece(idx)))),
                      sq, true, Piece(idx));
            ++sq;
        }
    ...
    if (!isdigit(ss.peek()) && !sfen) {
        ...
        while ((ss >> token) && !isspace(token)) {
            ...
            if (token == 'K')
                for (rsq = make_square(FILE_MAX, castling_rank(c)); piece_on(rsq) != rook;
                     --rsq) {
                }
            ...
            if (castling_enabled() && st->castlingKingSquare[c] == SQ_NONE) {
                Bitboard castlingKings =
                    pieces(c, castling_king_piece()) & rank_bb(castling_rank(c));
                ...
                st->castlingKingSquare[c] =
                    isChess960 && piece_on(rsq) == make_piece(c, castling_king_piece()) ? rsq
                    : castlingKings && (!more_than_one(castlingKings) || isChess960)
                        ? lsb(castlingKings)
            ...
            if (gating()) {
                ...
                else if (!seirawan_gating() || count_in_hand(c, ALL_PIECES) > 0 ||
                         captures_to_hand())
                    continue;
            }
        ...
        if (gating() && castling_enabled())
            for (Color c : { WHITE, BLACK })
                if ((gates(c) & pieces(castling_king_piece())) && !castling_rights(c) &&
...
void Position::set_castling_right(Color c, Square rfrom)
{
    ...
    Square kto = make_square(
        cr & KING_SIDE ? castling_kingside_file() : castling_queenside_file(), castling_rank(c));
...
void Position::set_check_info(StateInfo *si) const
{
    ...
    si->blockersForKing[WHITE] =
        slider_blockers(pieces(BLACK), count<KING>(WHITE) ? square<KING>(WHITE) : SQ_NONE,
                        si->pinners[BLACK], BLACK);
    si->blockersForKing[BLACK] =
        slider_blockers(pieces(WHITE), count<KING>(BLACK) ? square<KING>(BLACK) : SQ_NONE,
                        si->pinners[WHITE], WHITE);
    ...
    for (PieceType pt : piece_types()) {
        si->checkSquares[pt] =
            ksq != SQ_NONE ? attacks_bb(~sideToMove, pt, ksq, pieces()) : Bitboard(0);
    ...
    si->bikjang =
        var->bikjangRule && ksq != SQ_NONE
            ? bool(attacks_bb(sideToMove, ROOK, ksq, pieces()) & pieces(sideToMove, KING))
            : false;
...
void Position::set_state(StateInfo *si) const
{
    ...
    si->checkersBB = count<KING>(sideToMove) ? attackers_to(square<KING>(sideToMove), ~sideToMove)
                                             : Bitboard(0);
...
Position &Position::set(const string &code, Color c, StateInfo *si)
{
    ...
    string fenStr = n + "/" + sides[0] + char(FILE_NB - sides[0].length() + '0') + "/" + n + "/" +
                    n + "/" + n + "/" + n + "/" + sides[1] +
                    char(FILE_NB - sides[1].length() + '0') + "/" + n + " w - - 0 10";
...
string Position::fen(bool sfen, bool showPromoted, int countStarted, std::string holdings) const
{
        ...
        for (File f = FILE_A; f <= max_file(); ++f) {
            ...
            if (f <= max_file()) {
                if (unpromoted_piece_on(make_square(f, r)))
                    ...
                    // Set promoted pieces
                    if (((captures_to_hand() && !drop_loop()) || showPromoted) &&
                        is_promoted(make_square(f, r)))
                        ss << "~";
                }
            }
        }
        ...
        if (holdings != "-")
            ...
            for (Color c : { WHITE, BLACK })
                for (PieceType pt = KING; pt >= PAWN; --pt) {
                    ...
                    ss << std::string(pieceCountInHand[c][pt],
                                      piece_to_char()[make_piece(c, pt)]);
                }
        ss << ']';
    }
    ...
    if (chess960 && can_castle(WHITE_CASTLING) &&
        popcount(pieces(WHITE, castling_king_piece()) & rank_bb(castling_rank(WHITE))) > 1)
    ...
    if (gating() && gates(WHITE) &&
        (!seirawan_gating() || count_in_hand(WHITE, ALL_PIECES) > 0 || captures_to_hand()))
        for (File f = FILE_A; f <= max_file(); ++f)
            if ((gates(WHITE) & file_bb(f))
                // skip gating flags redundant with castling flags
                && !(!chess960 && can_castle(WHITE_CASTLING) &&
                     f == file_of(castling_king_square(WHITE))) &&
                !(can_castle(WHITE_OO) && f == file_of(castling_rook_square(WHITE_OO))) &&
                !(can_castle(WHITE_OOO) && f == file_of(castling_rook_square(WHITE_OOO))))
                ss << char('A' + f);

    // Disambiguation for chess960 "king" square
    if (chess960 && can_castle(BLACK_CASTLING) &&
        popcount(pieces(BLACK, castling_king_piece()) & rank_bb(castling_rank(BLACK))) > 1)
    ...
    if (gating() && gates(BLACK) &&
        (!seirawan_gating() || count_in_hand(BLACK, ALL_PIECES) > 0 || captures_to_hand()))
        for (File f = FILE_A; f <= max_file(); ++f)
            if ((gates(BLACK) & file_bb(f))
                // skip gating flags redundant with castling flags
                && !(!chess960 && can_castle(BLACK_CASTLING) &&
                     f == file_of(castling_king_square(BLACK))) &&
...
Bitboard Position::slider_blockers(Bitboard sliders, Square s, Bitboard &pinners, Color c) const
{
    ...
    if (var->fastAttacks)
        snipers = ((attacks_bb<ROOK>(s) & pieces(c, QUEEN, ROOK, CHANCELLOR)) |
                   (attacks_bb<BISHOP>(s) & pieces(c, QUEEN, BISHOP, ARCHBISHOP))) &
                  sliders;
    else
        for (PieceType pt : piece_types()) {
            Bitboard b =
                sliders & (PseudoAttacks[~c][pt][s] ^ LeaperAttacks[~c][pt][s]) & pieces(c, pt);
            if (b) {
    ...
    while (snipers) {
        ...
        if (b && (!more_than_one(b) ||
                  ((AttackRiderTypes[type_of(piece_on(sniperSq))] & HOPPING_RIDERS) &&
                   popcount(b) == 2))) {
...
Bitboard Position::attackers_to(Square s, Bitboard occupied, Color c,
...
bool Position::legal(Move m) const
{
    ...
    // Illegal checks
    if ((!checking_permitted() || (sittuyin_promotion() && type_of(m) == PROMOTION) ||
         (!drop_checks() && type_of(m) == DROP)) &&
        gives_check(m))
        return false;
    ...
    if (drop_opposite_colored_bishop() && type_of(m) == DROP) {
        if (type_of(moved_piece(m)) != BISHOP) {
            ...
            if (popcount(DarkSquares & (pieces(us, BISHOP) | remaining)) <
                    count_with_hand(us, BISHOP) / 2 ||
                popcount(~DarkSquares & (pieces(us, BISHOP) | remaining)) <
                    count_with_hand(us, BISHOP) / 2)
                return false;
        } else
            // Drop resulting in same-colored bishops
            if (popcount((DarkSquares & to ? DarkSquares : ~DarkSquares) & pieces(us, BISHOP)) +
                    1 >
                (count_with_hand(us, BISHOP) + 1) / 2)
                return false;
    }

    // No legal moves from target square
    if (immobility_illegal() && (type_of(m) == DROP || type_of(m) == NORMAL) &&
        !(moves_bb(us, type_of(moved_piece(m)), to, 0) & board_bb()))
        return false;
    ...
    if (var->extinctionPseudoRoyal) {
        ...
        if (type_of(m) == CASTLING) {
            ...
            kto = make_square(to > from ? castling_kingside_file() : castling_queenside_file(),
                              castling_rank(us));
            ...
            if (st->pseudoRoyals & from)
                for (Square s = from; s != kto; s += step)
                    if (!(blast_on_capture() &&
                          (attacks_bb<KING>(s) & st->pseudoRoyals & pieces(~sideToMove))) &&
        ...
        if (type_of(m) == PROMOTION &&
            extinction_piece_types().find(promotion_type(m)) != extinction_piece_types().end())
            pseudoRoyals |= kto;
    ...
    if (type_of(m) == CASTLING) {
        // After castling, the rook and king final positions are the same in
        // Chess960 as they would be in standard chess.
        to = make_square(to > from ? castling_kingside_file() : castling_queenside_file(),
                         castling_rank(us));
...
bool Position::pseudo_legal(const Move m) const
{
    ...
    if (type_of(m) == DROP)
        return piece_drops() && pc != NO_PIECE && color_of(pc) == us &&
    ...
    if (mandatory_piece_promotion() &&
        (is_promoted(from) ? piece_demotion()
                           : promoted_piece_type(type_of(pc)) != NO_PIECE_TYPE) &&
    ...
    if (type_of(pc) == PAWN) {
        ...
        if (mandatory_pawn_promotion() &&
            rank_of(to) == relative_rank(us, promotion_rank(), max_rank()))
            return false;
        ...
        if (!(pawn_attacks_bb(us, from) & pieces(~us) & to)  // Not a capture
            ...
            && !((from + 2 * pawn_push(us) == to)            // Not a double push
                 ...
                 empty(to) && empty(to - pawn_push(us)) && double_step_enabled()))
            return false;
    } else if (!((capture(m) ? attacks_from(us, type_of(pc), from)
                             : moves_from(us, type_of(pc), from)) &
                 to))
        return false;
...
bool Position::gives_check(Move m) const
{
    ...
    if (((type_of(m) != DROP && (blockers_for_king(~sideToMove) & from)) ||
         (non_sliding_riders() & pieces(sideToMove))) &&
        attackers_to(square<KING>(~sideToMove),
                     (type_of(m) == DROP ? pieces() : pieces() ^ from) | to, sideToMove,
                     janggiCannons))
        return true;

    // Is there a check by gated pieces?
    if (is_gating(m) &&
        attacks_bb(sideToMove, gating_type(m), gating_square(m), (pieces() ^ from) | to) &
    ...
    if (more_than_one(diagonal_lines() & (to | square<KING>(~sideToMove)))) {
        PieceType pt = type_of(moved_piece(m));
        PieceType diagType = pt == WAZIR     ? FERS
                             : pt == SOLDIER ? PAWN
                             : pt == ROOK    ? BISHOP
                                             : NO_PIECE_TYPE;
        Bitboard occupied = type_of(m) == DROP ? pieces() : pieces() ^ from;
        if (diagType &&
            (attacks_bb(sideToMove, diagType, to, occupied) & square<KING>(~sideToMove)))
            return true;
        else if (pt == JANGGI_CANNON &&
                 (rider_attacks_bb<RIDER_CANNON_DIAG>(to, occupied) &
    ...
    case PROMOTION:
        return attacks_bb(sideToMove, promotion_type(m), to, pieces() ^ from) &
               square<KING>(~sideToMove);

    case PIECE_PROMOTION:
        return attacks_bb(sideToMove, promoted_piece_type(type_of(moved_piece(m))), to,
                          pieces() ^ from) &
               square<KING>(~sideToMove);
    ...
    default:  // CASTLING
    {
        ...
        Square rfrom = to;
        Square kto =
            make_square(rfrom > kfrom ? castling_kingside_file() : castling_queenside_file(),
                        castling_rank(sideToMove));
        Square rto = kto + (rfrom > kfrom ? WEST : EAST);

        return (PseudoAttacks[sideToMove][type_of(piece_on(rfrom))][rto] &
                square<KING>(~sideToMove)) &&
               (attacks_bb(sideToMove, type_of(piece_on(rfrom)), rto,
                           (pieces() ^ kfrom ^ rfrom) | rto | kto) &
                square<KING>(~sideToMove));
    }
    }
}
...
void Position::do_move(Move m, StateInfo &newSt, bool givesCheck)
{
    ...
    if (captured) {
        ...
        if (captures_to_hand()) {
            ...
            k ^= Zobrist::inHand[pieceToHand]
                                [pieceCountInHand[color_of(pieceToHand)][type_of(pieceToHand)] -
                                 1] ^
                 Zobrist::inHand[pieceToHand]
                                [pieceCountInHand[color_of(pieceToHand)][type_of(pieceToHand)]];
    ...
    if (flip_enclosed_pieces() && !is_pass(m)) {
        ...
        if (flip_enclosed_pieces() == REVERSI) {
            Bitboard b = attacks_bb(us, QUEEN, to, board_bb() & ~pieces(~us)) &
                         ~PseudoAttacks[us][KING][to] & pieces(us);
            while (b)
        ...
        if (castling_dropped_piece() && rank_of(to) == castling_rank(us)) {
            if (type_of(pc) == castling_king_piece() && file_of(to) == castling_king_file()) {
                ...
                if ((file_of(to) == FILE_A || file_of(to) == max_file()) &&
                    piece_on(make_square(castling_king_file(), castling_rank(us))) ==
                        make_piece(us, castling_king_piece())) {
                    st->castlingKingSquare[us] =
                        make_square(castling_king_file(), castling_rank(us));
    ...
    if (type_of(pc) == PAWN) {
        ...
        if (type_of(m) != DROP && std::abs(int(to) - int(from)) == 2 * NORTH &&
            ...
            Piece promotion = make_piece(us, type_of(m) == PROMOTION ? promotion_type(m)
                                                                     : promoted_piece_type(PAWN));
        ...
        st->materialKey ^=
            Zobrist::psq[demotion][pieceCount[demotion] - 1] ^ Zobrist::psq[pc][pieceCount[pc]];
    ...
    if (captured && blast_on_capture()) {
        ...
        while (blast) {
            ...
            if (captures_to_hand()) {
                ...
                add_to_hand(pieceToHand);
                k ^= Zobrist::inHand[pieceToHand][pieceCountInHand[color_of(pieceToHand)]
                                                                  [type_of(pieceToHand)] -
                                                  1] ^
                     Zobrist::inHand[pieceToHand][pieceCountInHand[color_of(pieceToHand)]
                                                                  [type_of(pieceToHand)]];

                if (Eval::useNNUE) {
                    dp.handPiece[dp.dirty_num - 1] = pieceToHand;
                    dp.handCount[dp.dirty_num - 1] =
                        pieceCountInHand[color_of(pieceToHand)][type_of(pieceToHand)];
                }
            }
    ...
    if (counting_rule() &&
        ...
        st->countingPly = counting_rule() == MAKRUK_COUNTING && count<ALL_PIECES>(sideToMove) == 1
                              ? 2 * count<ALL_PIECES>()
                              : 0;
    }
...
void Position::undo_move(Move m)
{
    ...
    assert(type_of(m) == DROP || empty(from) || type_of(m) == CASTLING || is_gating(m) ||
           (type_of(m) == PROMOTION && sittuyin_promotion()) || (is_pass(m) && pass()));
    ...
    if (st->capturedPiece && blast_on_capture()) {
        ...
        while (blast) {
            ...
            Piece bpc = st->demotedBycatch & bsq
                            ? make_piece(color_of(unpromotedBpc),
            ...
            // Update board and piece lists
            if (bpc) {
                put_piece(bpc, bsq, isPromoted,
                          st->demotedBycatch & bsq ? unpromotedBpc : NO_PIECE);
                if (captures_to_hand())
    ...
    if (type_of(m) == CASTLING) {
        ...
        if (st->capturedPiece) {
            ...
            put_piece(st->capturedPiece, capsq, st->capturedpromoted,
                      st->unpromotedCapturedPiece);  // Restore the captured piece
            if (captures_to_hand())
    ...
    if (captured) {
        ...
        if (captures_to_hand()) {
            Piece removeFromHand = !drop_loop() && is_promoted(to)
                                       ? make_piece(~color_of(captured), PAWN)
                                       : ~captured;
            k ^= Zobrist::inHand[removeFromHand][pieceCountInHand[color_of(removeFromHand)]
                                                                 [type_of(removeFromHand)] +
                                                 1] ^
                 Zobrist::inHand[removeFromHand][pieceCountInHand[color_of(removeFromHand)]
                                                                 [type_of(removeFromHand)]];
        }
    }
    if (type_of(m) == DROP) {
...
Value Position::blast_see(Move m) const
{
    ...
    if (!capture(m)) {
        ...
        while (attackers) {
            ...
            if (extinction_piece_types().find(type_of(piece_on(s))) ==
                extinction_piece_types().end())
                minAttacker = std::min(
                    minAttacker, blast & s ? VALUE_ZERO : CapturePieceValue[MG][piece_on(s)]);
        }
...
bool Position::see_ge(Move m, Value threshold) const
{
    ...
    // Extinction
    if (extinction_value() != VALUE_NONE && piece_on(to) &&
        ((extinction_piece_types().find(type_of(piece_on(to))) !=
              extinction_piece_types().end() &&
    ...
    if (must_capture() || !checking_permitted() || is_gating(m) ||
    ...
    if (type_of(moved_piece(m)) == JANGGI_CANNON &&
        !(attackers & pieces(~stm) & ~pieces(JANGGI_CANNON)))
...
bool Position::is_optional_game_end(Value &result, int ply, int countStarted) const
{
    ...
    if (n_move_rule() && st->rule50 > (2 * n_move_rule() - 1) &&
        ...
        result = var->materialCounting ? convert_mate_value(material_counting_result(), ply)
                                       : VALUE_DRAW;
        return true;
    }
    ...
    if (n_fold_rule()) {
        int end =
            captures_to_hand() ? st->pliesFromNull : std::min(st->rule50, st->pliesFromNull);
        ...
        if (end >= 4) {
            ...
            int moveRepetition =
                var->moveRepetitionIllegal && type_of(st->move) == NORMAL &&
                        !st->previous->checkersBB && !stp->previous->checkersBB &&
                        (board_bb(~side_to_move(), type_of(piece_on(to_sq(st->move)))) &
                         board_bb(side_to_move(), KING))
                    ? (stp->move == reverse_move(st->move) ? 2
                       : is_pass(stp->move)                ? 1
                                                           : 0)
                    : 0;
            ...
            for (int i = 4; i <= end; i += 2) {
                ...
                if (moveRepetition > 0) {
                    if (i + 1 <= end && stp->previous->previous->previous->checkersBB)
                        ...
                        if (stp->previous->previous->move ==
                            reverse_move((moveRepetition == 1 ? st : stp)->move))
                            moveRepetition++;
                        else
                            moveRepetition = 0;
                    } else {
                        assert(moveRepetition == 4);
                        if (!stp->previous->previous->capturedPiece &&
                            from_sq(stp->move) == to_sq(stp->previous->previous->move)) {
                            result = VALUE_MATE;
                            return true;
                        } else
                            moveRepetition = 0;
                ...
                if (stp->key == st->key &&
                    ...
                    result = convert_mate_value(
                        var->perpetualCheckIllegal && perpetualThem      ? VALUE_MATE
    ...
    // counting rules
    if (counting_rule() && st->countingLimit && counting_ply(countStarted) > st->countingLimit &&
        (!checkers() || MoveList<LEGAL>(*this).size())) {
        result = VALUE_DRAW;
        return true;
    }
    ...
    if (sittuyin_promotion() && count<ALL_PIECES>(sideToMove) == 2 &&
        count<PAWN>(sideToMove) == 1 && !checkers()) {
...
bool Position::is_immediate_game_end(Value &result, int ply) const
{
    ...
    // capture the flag
    if (capture_the_flag_piece() && flag_move() &&
    ...
    if (capture_the_flag_piece() && (!flag_move() || capture_the_flag_piece() == KING) &&
        (capture_the_flag(~sideToMove) & pieces(~sideToMove, capture_the_flag_piece()))) {
        bool gameEnd = true;
        // Check whether king can move to CTF zone
        if (flag_move() && sideToMove == BLACK && !checkers() && count<KING>(sideToMove) &&
            (capture_the_flag(sideToMove) &
             attacks_from(sideToMove, KING, square<KING>(sideToMove)))) {
            assert(capture_the_flag_piece() == KING);
            gameEnd = true;
            for (const auto &m : MoveList<NON_EVASIONS>(*this))
                if (type_of(moved_piece(m)) == KING &&
                    (capture_the_flag(sideToMove) & to_sq(m)) && legal(m)) {
                    gameEnd = false;
                    break;
                }
        }
    ...
    if (st->pliesFromNull > 0 &&
        ((st->bikjang && st->previous->bikjang) || (st->pass && st->previous->pass))) {
        result = var->materialCounting ? convert_mate_value(material_counting_result(), ply)
                                       : VALUE_DRAW;
        return true;
    }
...
bool Position::has_game_cycle(int ply) const
{
    ...
    if (end < 3 || var->nFoldValue != VALUE_DRAW || var->perpetualCheckIllegal ||
        var->materialCounting || var->moveRepetitionIllegal)
        return false;
...
bool Position::pos_is_ok() const
{
    ...
    if (pieceCount[make_piece(WHITE, PAWN)] > 64 || pieceCount[make_piece(BLACK, PAWN)] > 64)
