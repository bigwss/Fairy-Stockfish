...
{
    int rd = edge_distance(rank_of(s), pos.max_rank()),
        fd = edge_distance(file_of(s), pos.max_file());
...
template <> Value Endgame<KXK>::operator()(const Position &pos) const
{
    ...
    Value result = pos.non_pawn_material(strongSide) + pos.count<PAWN>(strongSide) * PawnValueEg +
                   push_to_edge(weakKing, pos) + push_close(strongKing, weakKing);
    ...
    if (pos.count<QUEEN>(strongSide) || pos.count<ROOK>(strongSide) ||
        ...
        (pos.count<FERS>(strongSide) >= 3 && (DarkSquares & pos.pieces(strongSide, FERS)) &&
...
template <> Value Endgame<KBNK>::operator()(const Position &pos) const
{
    ...
    Value result = (VALUE_KNOWN_WIN + 3520) + push_close(strongKing, weakKing) +
                   420 * push_to_corner(opposite_colors(strongBishop, SQ_A1)
                                            ? flip_file(weakKing, pos.max_file())
                                            : weakKing,
                                        pos);
...
template <> Value Endgame<KPK>::operator()(const Position &pos) const
{
    ...
    if (pos.promotion_rank() != RANK_8 || RANK_MAX != RANK_8 ||
...
template <> Value Endgame<KQKR>::operator()(const Position &pos) const
{
    ...
    Value result = QueenValueEg - RookValueEg + push_to_edge(weakKing, pos) +
...
template <> Value Endgame<KNNKP>::operator()(const Position &pos) const
{
    ...
    Value result =
        PawnValueEg + 2 * push_to_edge(weakKing, pos) - 10 * relative_rank(weakSide, weakPawn);
...
/// KFsPs vs K.
template <> Value Endgame<KFsPsK>::operator()(const Position &pos) const
{
    ...
    Value result = pos.non_pawn_material(strongSide) + pos.count<PAWN>(strongSide) * PawnValueEg +
                   push_to_edge(loserKSq, pos) + push_close(winnerKSq, loserKSq);

    if (pos.count<FERS>(strongSide) >= 3 && (DarkSquares & pos.pieces(strongSide, FERS)) &&
        (~DarkSquares & pos.pieces(strongSide, FERS)))
        result = std::min(result + VALUE_KNOWN_WIN, VALUE_TB_WIN_IN_MAX_PLY - 1);
        ...
        while (b && (!dark || !light)) {
            if (file_of(pop_lsb(b)) % 2 !=
                relative_rank(strongSide, pos.promotion_rank(), pos.max_rank()) % 2)
                light = true;
            else
                dark = true;
        }
...
/// Mate with KNS vs K.
template <> Value Endgame<KNSK>::operator()(const Position &pos) const
{
    ...
    Value result =
        VALUE_KNOWN_WIN + push_close(winnerKSq, loserKSq) +
...
template <> Value Endgame<KNFK>::operator()(const Position &pos) const
{
    ...
    Value result = Value(push_close(winnerKSq, loserKSq)) + 50 * push_to_corner(loserKSq, pos);

    return strongSide == pos.side_to_move() ? result : -result;
}

/// KNSFKR vs K.
template <> Value Endgame<KNSFKR>::operator()(const Position &pos) const
{
    ...
    Value result =
        KnightValueEg + SilverValueEg + FersValueEg - RookValueEg +
...
/// Mate with KSF vs K.
template <> Value Endgame<KSFK>::operator()(const Position &pos) const
{
...
/// Mate with KSF vs KF.
template <> Value Endgame<KSFKF>::operator()(const Position &pos) const
{
    ...
    Value result =
        SilverValueEg + push_close(winnerKSq, loserKSq) + push_away(fersSq, loserKSq) +
        push_to_opposing_edge(relative_square(strongSide, loserKSq, pos.max_rank()), pos);
...
/// KR vs KS
template <> Value Endgame<KRKS>::operator()(const Position &pos) const
{
    ...
    Value result = RookValueEg - SilverValueEg + push_to_edge(loserKSq, pos) +
                   push_close(winnerKSq, loserKSq);
