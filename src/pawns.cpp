...
constexpr Score BlockedStorm[RANK_NB] = { S(0, 0),   S(0, 0),  S(75, 78), S(-8, 16),
                                          S(-6, 10), S(-6, 6), S(0, 2) };
...
constexpr Score KingOnFile[2][2] = { { S(-21, 10), S(-7, 1) }, { S(0, -3), S(9, -4) } };
...
template <Color Us> Score evaluate(const Position &pos, Pawns::Entry *e)
{
    ...
    while (b) {
        ...
        leverPush = relative_rank(Them, s, pos.max_rank()) > RANK_1
                        ? theirPawns & pawn_attacks_bb(Us, s + Up)
                        : Bitboard(0);
        ...
        backward = is_ok(s + Up) && !(neighbours & forward_ranks_bb(Them, s + Up)) &&
                   (stoppers & blocked);
        ...
        if (passed && is_ok(s + Up) &&
            (r < pos.promotion_rank() || !pos.mandatory_pawn_promotion()))
            e->passedPawns[Us] |= s;
        ...
        if ((support | phalanx) &&
            ...
            int v = Connected[r] * (2 + bool(phalanx) - bool(opposed)) *
                        (r == RANK_2 && pos.captures_to_hand() ? 3 : 1) +
                    22 * popcount(support);
...
template <Color Us> Score Entry::evaluate_shelter(const Position &pos, Square ksq) const
{
    ...
    for (File f = File(center - 1); f <= File(center + 1); ++f) {
        ...
        bonus += make_score(ShelterStrength[d][ourRank], 0) *
                 (1 + (pos.captures_to_hand() && ourRank <= RANK_2) +
...
template <Color Us> Score Entry::do_king_safety(const Position &pos)
{
    ...
    if (pos.can_castle(Us & KING_SIDE))
        shelter = std::max(shelter,
                           evaluate_shelter<Us>(pos, make_square(pos.castling_kingside_file(),
                                                                 pos.castling_rank(Us))),
                           compare);

    if (pos.can_castle(Us & QUEEN_SIDE))
        shelter = std::max(shelter,
                           evaluate_shelter<Us>(pos, make_square(pos.castling_queenside_file(),
                                                                 pos.castling_rank(Us))),
                           compare);
