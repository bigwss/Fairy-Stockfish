...
Variant *minixiangqi_variant()
{
    ...
    v->mobilityRegion[WHITE][KING] =
        (Rank1BB | Rank2BB | Rank3BB) & (FileCBB | FileDBB | FileEBB);
    v->mobilityRegion[BLACK][KING] =
        (Rank5BB | Rank6BB | Rank7BB) & (FileCBB | FileDBB | FileEBB);
    v->kingType = WAZIR;
    v->promotionPieceTypes = {};
    v->doubleStep = false;
    v->castling = false;
...
Variant *amazons_variant()
{
    ...
    v->startFen = "3q2q3/10/10/q8q/10/10/Q8Q/10/10/"
                  "3Q2Q3["
                  "PPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPpppppppppppppppppppppppppppppppp"
                  "pppppppppppppp] w - - 0 1";
    v->stalemateValue = -VALUE_MATE;
    v->arrowGating = true;
    return v;
}
...
Variant *xiangqi_variant()
{
    ...
    v->mobilityRegion[WHITE][KING] =
        (Rank1BB | Rank2BB | Rank3BB) & (FileDBB | FileEBB | FileFBB);
    v->mobilityRegion[BLACK][KING] =
        (Rank8BB | Rank9BB | Rank10BB) & (FileDBB | FileEBB | FileFBB);
...
Variant *janggi_variant()
{
    ...
    v->diagonalLines =
        make_bitboard(SQ_D1, SQ_F1, SQ_E2, SQ_D3, SQ_F3, SQ_D8, SQ_F8, SQ_E9, SQ_D10, SQ_F10);
    v->pass = true;
...
template <bool DoCheck> void VariantMap::parse_istream(std::istream &file)
{
    ...
    while (file.get() && std::getline(std::getline(file, variant, ']'), input)) {
        ...
        if (!std::getline(std::getline(std::stringstream(variant), variant, ':'),
                          variant_template))
            variant_template = "";
        ...
        if (variants.find(variant) != variants.end())
            ...
            std::cerr << "Variant template '" << variant_template << "' does not exist."
                      << std::endl;
        else {
            if (DoCheck)
                std::cerr << "Parsing variant: " << variant << std::endl;
            Variant *v =
                !variant_template.empty()
            ...
            if (v->maxFile <= FILE_MAX && v->maxRank <= RANK_MAX) {
                ...
                // even when only checking them, but we remove them later after parsing is
                // finished.
                if (DoCheck)
...
template <bool DoCheck> void VariantMap::parse(std::string path)
{
