...
inline bool is_move_none(Move move, const std::string &strMove, const Position &pos)
{
    if (move == MOVE_NONE) {
        ...
        std::cerr << " '" << strMove << "' for position '" << pos.fen() << "' is invalid."
                  << std::endl;
        return true;
    }
    return false;
...
public:
    ...
    // TODO: This is a naive implementation which compares all legal SAN moves with the requested
    // string. If the SAN move wasn't found the position remains unchanged. Alternatively,
    ...
    {
        return Stockfish::has_insufficient_material(WHITE, pos) &&
               Stockfish::has_insufficient_material(BLACK, pos);
    }
...
template <typename T> void set_option(std::string name, T value)
{
    Options[name] = value;
...
Game read_game_pgn(std::string pgn)
{
    ...
    while (true) {
        ...
        if (!headersParsed && pgn[lineStart] == '[') {
            ...
            // put item into list
            game.header[pgn.substr(headerKeyStart, headerKeyEnd - headerKeyStart)] =
            ...
            while (curIdx <= lineEnd) {
                ...
                if (sanMoveEnd > curIdx) {
                    ...
                    if (annotationChar1 != std::string::npos ||
                        annotationChar2 != std::string::npos)
...
EMSCRIPTEN_BINDINGS(ffish_js)
{
    class_<Board>("Board")
        ...
        .function("sanMove",
                  select_overload<std::string(std::string, Notation)>(&Board::san_move))
        .function("variationSan",
                  select_overload<std::string(std::string)>(&Board::variation_san))
        .function("variationSan",
                  select_overload<std::string(std::string, Notation)>(&Board::variation_san))
        .function("variationSan", select_overload<std::string(std::string, Notation, bool)>(
                                      &Board::variation_san))
        ...
        .function("pushSanMoves",
                  select_overload<void(std::string, Notation)>(&Board::push_san_moves))
    ...
    function("validateFen",
             select_overload<int(std::string, std::string, bool)>(&ffish::validate_fen));
