...
{
    target = value == "reversi" ? REVERSI : value == "ataxx" ? ATAXX : NO_ENCLOSING;
...
template <> bool set(const std::string &value, Bitboard &target)
{
    ...
    while (!ss.eof() && ss >> file && ss >> rank)
        target |= file == '*' ? rank_bb(Rank(rank - 1))
                              : square_bb(make_square(File(tolower(file) - 'a'), Rank(rank - 1)));
    return !ss.fail();
}
...
template <class T>
void VariantParser<DoCheck>::parse_attribute(const std::string &key, T &target)
{
...
template <bool DoCheck>
void VariantParser<DoCheck>::parse_attribute(const std::string &key, PieceType &target,
                                             std::string pieceToChar)
{
    ...
    if (it != config.end()) {
        ...
        if (ss >> token &&
            (idx = token == '-' ? 0 : pieceToChar.find(toupper(token))) != std::string::npos)
...
template <bool DoCheck> Variant *VariantParser<DoCheck>::parse()
{
...
template <bool DoCheck> Variant *VariantParser<DoCheck>::parse(Variant *v)
{
    // piece types
    for (PieceType pt = PAWN; pt <= KING; ++pt) {
        ...
        for (Color c : { WHITE, BLACK }) {
            ...
            parse_attribute("mobilityRegion" + color + capitalizedPiece,
                            v->mobilityRegion[c][pt]);
        }
    }
    // piece values
    ...
    if (it_ext != config.end()) {
        ...
        while (ss >> token &&
               (idx = token == '*' ? size_t(ALL_PIECES) : v->pieceToChar.find(toupper(token))) !=
                   std::string::npos)
    ...
    if (DoCheck) {
        ...
        for (PieceType pt : v->pieceTypes) {
            for (Color c : { WHITE, BLACK })
                if (std::count(v->pieceToChar.begin(), v->pieceToChar.end(),
                               v->pieceToChar[make_piece(c, pt)]) != 1)
        ...
        if (v->doubleStep && v->doubleStepRankMin > v->doubleStepRank)
            std::cerr << "Inconsistent settings: doubleStepRankMin > doubleStepRank."
                      << std::endl;
        ...
        if (v->pieceTypes.find(KING) != v->pieceTypes.end()) {
            ...
            if (!is_custom(v->kingType)) {
                ...
                if (pi->hopper[MODALITY_QUIET].size() || pi->hopper[MODALITY_CAPTURE].size() ||
                    std::any_of(
                        pi->steps[MODALITY_CAPTURE].begin(), pi->steps[MODALITY_CAPTURE].end(),
