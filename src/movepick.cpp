...
MovePicker::MovePicker(const Position &p, Move ttm, Depth d, const ButterflyHistory *mh,
                       const LowPlyHistory *lp, const CapturePieceToHistory *cph,
                       const PieceToHistory **ch, Move cm, const Move *killers, int pl)
...
template <GenType Type> void MovePicker::score()
{
    ...
    for (auto &m : *this)
        if constexpr (Type == CAPTURES)
            m.value =
                int(PieceValue[MG][pos.piece_on(to_sq(m))]) * 6 +
        ...
        else  // Type == EVASIONS
        {
            if (pos.capture(m))
                m.value =
                    PieceValue[MG][pos.piece_on(to_sq(m))] - Value(type_of(pos.moved_piece(m)));
            else
                m.value =
                    (*mainHistory)[pos.side_to_move()][from_to(m)] +
                    2 * (*continuationHistory[0])[history_slot(pos.moved_piece(m))][to_sq(m)] -
...
template <MovePicker::PickType T, typename Pred> Move MovePicker::select(Pred filter)
{
