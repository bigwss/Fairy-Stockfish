...
/// _WIN64             Building on Windows 64 bit
    ...
    #define ASSERT_ALIGNED(ptr, alignment)                                                       \
        ...
        #ifdef LARGEBOARDS
            #define pext(b, m)                                                                   \
                (_pext_u64(b, m) ^ (_pext_u64(b >> 64, m >> 64) << popcount((m << 64) >> 64)))
        #else
...
struct Bitboard {
    ...
    inline Bitboard operator*(const Bitboard x) const
    {
        ...
        return Bitboard(b64[0] * x.b64[1] + b64[1] * x.b64[0] + (a_hi * b_hi) + (t1 >> 32) +
                            (t2 >> 32),
...
{
    return v == VALUE_MATE ? mate_in(ply) : v == -VALUE_MATE ? mated_in(ply) : v;
}
...
template <MoveType T> inline Move make(Square from, Square to, PieceType pt = NO_PIECE_TYPE)
{
...
inline int dist(Direction d)
{
    return std::abs(d % NORTH) < NORTH / 2
               ? std::max(std::abs(d / NORTH), int(std::abs(d % NORTH)))
