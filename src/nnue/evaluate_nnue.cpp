...
template <typename T> bool write_parameters(std::ostream &stream, const T &reference)
{
...
bool save_eval(const std::optional<std::string> &filename)
{
    ...
    if (filename.has_value())
        ...
        if (eval_file_loaded != EvalFileDefaultName) {
            msg = "Failed to export a net. A non-embedded net can only be saved if the filename "
                  "is specified";
