...
private:
    ...
    // Actual input dimensions (determined at runtime when loading model)
    IndexType actualInputDimensions;
...
public:
    ...
    // Constructor
    FeatureTransformer()
            : actualInputDimensions(0), biases(nullptr), weights(nullptr), psqtWeights(nullptr)
    {
    }
    ...
    {
        if (biases)
            std_aligned_free(biases);
        if (weights)
            std_aligned_free(weights);
        if (psqtWeights)
            std_aligned_free(psqtWeights);
    }
    ...
    {
        // Get actual input dimensions from the feature set
        if (actualInputDimensions == 0 || actualInputDimensions != FeatureSet::get_dimensions()) {
            ...
            actualInputDimensions = FeatureSet::get_dimensions();
            // Calculate memory sizes
            const size_t biasesSize = HalfDimensions * sizeof(BiasType);
            ...
            std::cout << "  biases size: " << biasesSize << " bytes" << std::endl;
            std::cout << "  weights size: " << weightsSize << " bytes" << std::endl;
            std::cout << "  psqtWeights size: " << psqtWeightsSize << " bytes" << std::endl;
            std::cout << "  Total size: " << totalSize << " bytes ("
                      << (totalSize / 1024.0 / 1024.0) << " MB)" << std::endl;
#endif
            // Allocate memory with proper alignment
            biases = reinterpret_cast<BiasType *>(std_aligned_alloc(CacheLineSize, biasesSize));
            weights =
                reinterpret_cast<WeightType *>(std_aligned_alloc(CacheLineSize, weightsSize));
            psqtWeights = reinterpret_cast<PSQTWeightType *>(
                std_aligned_alloc(CacheLineSize, psqtWeightsSize));

            if (!biases || !weights || !psqtWeights) {
        ...
        read_little_endian<WeightType>(stream, weights, HalfDimensions * actualInputDimensions);
        read_little_endian<PSQTWeightType>(stream, psqtWeights,
                                           PSQTBuckets * actualInputDimensions);
    ...
    bool write_parameters(std::ostream &stream) const
    {
        ...
        write_little_endian<WeightType>(stream, weights, HalfDimensions * actualInputDimensions);
...
private:
    void update_accumulator(const Position &pos, const Color perspective) const
    {
        ...
        if (st->accumulator.computed[perspective]) {
            ...
            FeatureSet::append_changed_indices(ksq, next, perspective, removed[0], added[0], pos);
            for (StateInfo *st2 = pos.state(); st2 != next; st2 = st2->previous)
                FeatureSet::append_changed_indices(ksq, st2, perspective, removed[1], added[1],
                                                   pos);
            ...
            // Now update the accumulators listed in states_to_update[], where the last element is
            // a sentinel.
    ...
    alignas(CacheLineSize) BiasType *biases;
    alignas(CacheLineSize) WeightType *weights;
    alignas(CacheLineSize) PSQTWeightType *psqtWeights;
};
