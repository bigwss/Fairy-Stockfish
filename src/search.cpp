...
{
    return (3 + depth * depth + 2 * pos.blast_on_capture()) /
           (2 - improving + pos.blast_on_capture());
}
...
struct Skill {
    ...
    {
        return depth == 1 + std::max(level, 0);
    }
...
void MainThread::search()
{
    ...
    if (rootMoves.empty() ||
        (Options["Protocol"] == "xboard" && rootPos.is_optional_game_end())) {
        ...
        if (Options["Protocol"] == "xboard") {
            ...
            sync_cout << (result == VALUE_DRAW ? "1/2-1/2 {Draw}"
                          : (rootPos.side_to_move() == BLACK ? -result : result) == VALUE_MATE
                              ? "1-0 {White wins}"
                              : "0-1 {Black wins}")
                      << sync_endl;
        } else
    ...
    if (Options["Protocol"] == "xboard") {
        ...
        if (rootPos.two_boards() && rootPos.virtual_drop(bestMove)) {
            Partner.ptell("fast");
            while (!Threads.abort && !Partner.partnerDead && !Partner.fast &&
    ...
    if (Options["MultiMoves"]) {
        // "|" as a separator
        size_t multiPV = size_t(Options["MultiPV"]);
        multiPV = std::min(multiPV, bestThread->rootMoves.size());
        sync_cout << "multimoves";
        std::ostringstream oss;
        for (size_t pvIndex = 0; pvIndex < multiPV; ++pvIndex) {
            oss << " | ";
            Value v = bestThread->rootMoves[pvIndex].score;
            std::string wdlstr = UCI::wdl(v, rootPos.game_ply());
            std::string action = UCI::move(rootPos, bestThread->rootMoves[pvIndex].pv[0]);
            oss << action;
            oss << " score " << UCI::value(bestThread->rootMoves[pvIndex].score);
            oss << wdlstr;
            oss << " pv";
            for (std::size_t i = 0; i < bestThread->rootMoves[pvIndex].pv.size(); ++i) {
                oss << " " << UCI::move(rootPos, bestThread->rootMoves[pvIndex].pv[i]);
            }
        }
        std::cout << oss.str() << sync_endl;
    } else {
#if 0
        // Original Logic
        ...
        std::cout << sync_endl;
#else
        // like this: bestmove g3g4 score cp 0 wdl 41 917 42 pv h9h3 c2c6
        sync_cout << "bestmove " << UCI::move(rootPos, bestThread->rootMoves[0].pv[0]);
        ...
        std::cout << wdlstr;
        std::cout << " pv";
        for (std::size_t i = 0; i < bestThread->rootMoves[0].pv.size(); ++i) {
            std::cout << " " << UCI::move(rootPos, bestThread->rootMoves[0].pv[i]);
        }
        std::cout << sync_endl;
#endif
    }
    ...
    //         std::string action = UCI::move(rootPos, bestThread->rootMoves[pvIndex].pv[0]);
    //         std::cout << action;
    //         std::cout << " score " << UCI::value(bestThread->rootMoves[pvIndex].score);
    //         std::cout << wdlstr;
    //         std::cout << " pv";
    //         for (std::size_t i = 1; i < bestThread->rootMoves[pvIndex].pv.size(); ++i) {
    //             std::cout << " " << UCI::move(rootPos, bestThread->rootMoves[pvIndex].pv[i]);
    //         }
    //         std::cout << sync_endl;
    //     } else {
    ...
    //         if (pvIndex == 0) {
    //             sync_cout << "bestmove "
    //                       << UCI::move(rootPos, bestThread->rootMoves[pvIndex].pv[0]);
    //             if (bestThread->rootMoves[pvIndex].pv.size() > 1 ||
    //                 bestThread->rootMoves[pvIndex].extract_ponder_from_tt(rootPos))
    //                 std::cout << " ponder "
    //                           << UCI::move(rootPos, bestThread->rootMoves[pvIndex].pv[1]);
    //             std::cout << sync_endl;
    //         }
    //     }
    // }
    ...
    while (++rootDepth < MAX_PLY && !Threads.stop &&
        ...
        for (pvIdx = 0; pvIdx < multiPV && !Threads.stop; ++pvIdx) {
            ...
            while (true) {
                ...
                bestValue =
                    Stockfish::search<Root>(rootPos, ss, alpha, beta, adjustedDepth, false);
        ...
        if (Limits.use_time_management() && !Threads.stop && !mainThread->stopOnPonderhit) {
            ...
            // Cap used time in case of a single legal move for a better viewer experience in
            // tournaments yielding correct scores and sufficiently fast moves.
            if (rootMoves.size() == 1)
            ...
            if (completedDepth >= 8 && rootPos.two_boards() && Options["Protocol"] == "xboard") {
                ...
                if (Limits.time[us])
                    Partner.ptell<FAIRY>("time " +
                                         std::to_string((Limits.time[us] - Time.elapsed()) / 10));
                if (Limits.time[~us])
                ...
                else if (!Partner.weWin && bestValue >= VALUE_MATE_IN_MAX_PLY &&
                         Limits.time[~us] < Partner.time) {
                    Partner.ptell("sit");
                    Partner.weWin = true;
                }
                // We are no longer winning
                else if (Partner.weWin &&
                         (bestValue < VALUE_MATE_IN_MAX_PLY || Limits.time[~us] > Partner.time)) {
                    Partner.ptell("x");
                    Partner.weWin = false;
                }
                // We can win if partner delivers required material quickly
                else if (!Partner.weVirtualWin && bestValue >= VALUE_VIRTUAL_MATE_IN_MAX_PLY &&
                ...
                // Virtual mate threat is over
                         ...
                         (bestValue > -VALUE_VIRTUAL_MATE_IN_MAX_PLY ||
                          bestValue < -VALUE_VIRTUAL_MATE || Limits.time[~us] < Partner.time)) {
                    Partner.ptell("x");
...
Value search(Position &pos, Stack *ss, Value alpha, Value beta, Depth depth, bool cutNode)
{
    ...
    // Update low ply history for previous move if we are near root and position is or has been in
    // PV
    ...
    if (!rootNode && TB::Cardinality) {
        ...
        if (piecesCount <= TB::Cardinality &&
            ...
            pos.rule50_count() == 0 && Options["UCI_Variant"] == "chess" &&
    ...
    if (is_ok((ss - 1)->currentMove) && !(ss - 1)->inCheck && !priorCapture) {
        int bonus =
            std::clamp(-depth * 4 * int((ss - 1)->staticEval + ss->staticEval), -1000, 1000);
    ...
    if (!PvNode && depth < 9 - 3 * pos.blast_on_capture() &&
        eval - futility_margin(depth, improving) *
                    (1 + pos.check_counting() + 2 * pos.must_capture() +
                     pos.extinction_single_piece() + !pos.checking_permitted()) >=
            beta &&
    ...
    if (!PvNode && (ss - 1)->currentMove != MOVE_NULL && (ss - 1)->statScore < 23767 &&
        ...
        ss->staticEval >=
            beta - 20 * depth - 22 * improving + 168 * ss->ttPv + 159 +
                200 * (!pos.double_step_enabled() && pos.piece_to_char()[PAWN] != ' ') &&
        ...
        Depth R =
            (1090 - 300 * pos.must_capture() - 250 * !pos.checking_permitted() + 81 * depth) /
                256 +
            std::min(int(eval - beta) / 205,
                     pos.must_capture() || pos.blast_on_capture() ? 0 : 3);
    ...
    probCutBeta = beta +
                  (209 + 20 * !!pos.capture_the_flag_piece() + 50 * pos.captures_to_hand()) *
    ...
    if (!PvNode && depth > 4 &&
        ...
        // there and in further interactions with transposition table cutoff depth is set to depth
        // - 3 because probCut search has depth set to depth - 4 but we also do a move before it
        ...
        while ((move = mp.next_move()) != MOVE_NONE && probCutCount < 2 + 2 * cutNode)
            if (move != excludedMove && pos.legal(move)) {
                ...
                ss->continuationHistory =
                    &thisThread
                         ->continuationHistory[ss->inCheck][captureOrPromotion]
                                              [history_slot(pos.moved_piece(move))][to_sq(move)];
                ...
                if (value >= probCutBeta) {
                    // if transposition table doesn't have equal or more deep info write probCut
                    // data into it
...
moves_loop:  // When in check, search starts from here
    ...
    if (ss->inCheck && !PvNode && depth >= 4 && ttCapture && (tte->bound() & BOUND_LOWER) &&
        tte->depth() >= depth - 3 && ttValue >= probCutBeta && abs(ttValue) <= VALUE_KNOWN_WIN &&
    ...
    // at a depth equal or greater than the current depth, and the result of this search was a
    // fail low.
    bool likelyFailLow =
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE) {
        ...
        if (rootNode && thisThread == Threads.main() && Time.elapsed() > 3000 &&
            Options["Protocol"] != "xboard")
            sync_cout << "info depth " << depth << " currmove " << UCI::move(pos, move)
        ...
        if (!rootNode &&
            ...
            if (pos.must_capture() && pos.attackers_to(to_sq(move), ~us)) {
                ...
                // SEE based pruning
                if (!pos.see_ge(move,
                                Value(-218 - 120 * pos.captures_to_hand()) * depth))  // (~25 Elo)
                    continue;
            } else {
                ...
                if (lmrDepth < 7 && !ss->inCheck && !pos.extinction_single_piece() &&
                    ss->staticEval + (174 + 157 * lmrDepth) * (1 + pos.check_counting()) <=
                        alpha &&
                    (*contHist[0])[history_slot(movedPiece)][to_sq(move)] +
                            (*contHist[1])[history_slot(movedPiece)][to_sq(move)] +
                            (*contHist[3])[history_slot(movedPiece)][to_sq(move)] +
                            (*contHist[5])[history_slot(movedPiece)][to_sq(move)] / 3 <
                        28255)
                    continue;

                // Prune moves with negative SEE (~20 Elo)
                if (!pos.see_ge(move, Value(-(30 - std::min(lmrDepth, 18) +
                                              10 * !!pos.capture_the_flag_piece()) *
                                            lmrDepth * lmrDepth)))
                    continue;
            }
        }
        ...
        if (!rootNode && depth >= 7 - 2 * (pos.count<KING>() == 1) && move == ttMove &&
        ...
        else if (pos.must_capture() && pos.capture(move) &&
        ...
        ss->continuationHistory =
            &thisThread->continuationHistory[ss->inCheck][captureOrPromotion]
                                            [history_slot(movedPiece)][to_sq(move)];
        ...
        if (depth >= 3 && moveCount > 1 + 2 * rootNode &&
            ...
            // Increase reduction at root and non-PV nodes when the best move does not change
            // frequently
            ...
            if (!captureOrPromotion) {
                ...
                ss->statScore = thisThread->mainHistory[us][from_to(move)] +
                                (*contHist[0])[history_slot(movedPiece)][to_sq(move)] +
                                (*contHist[1])[history_slot(movedPiece)][to_sq(move)] +
                                (*contHist[3])[history_slot(movedPiece)][to_sq(move)] - 4923;
            ...
            Depth d = std::clamp(newDepth - r, 1,
        ...
        if (PvNode && (moveCount == 1 || (value > alpha && (rootNode || value < beta)))) {
            ...
            value =
                -search<PV>(pos, ss + 1, -beta, -alpha, std::min(maxNextDepth, newDepth), false);
        }
        ...
        // If the move is worse than some previously searched move, remember it to update its
        // stats later
        if (move != bestMove) {
    ...
    */
    ...
    // If there is a move which produces search value greater than alpha we update stats of
    // searched moves
    else if (bestMove)
...
Value qsearch(Position &pos, Stack *ss, Value alpha, Value beta, Depth depth)
{
    ...
    while ((move = mp.next_move()) != MOVE_NONE) {
        ...
        if (bestValue > VALUE_TB_LOSS_IN_MAX_PLY && !givesCheck &&
            !(pos.extinction_value() == -VALUE_MATE && pos.piece_on(to_sq(move)) &&
              pos.extinction_piece_types().find(type_of(pos.piece_on(to_sq(move)))) !=
                  pos.extinction_piece_types().end()) &&
        ...
        if (!captureOrPromotion && bestValue > VALUE_TB_LOSS_IN_MAX_PLY &&
            (*contHist[0])[history_slot(pos.moved_piece(move))][to_sq(move)] <
                CounterMovePruneThreshold &&
            (*contHist[1])[history_slot(pos.moved_piece(move))][to_sq(move)] <
                CounterMovePruneThreshold)
            continue;
...
void MainThread::check_time()
{
    ...
    if (rootPos.two_boards() && Time.elapsed() < Limits.time[rootPos.side_to_move()] - 1000 &&
        (Partner.sitRequested || (Partner.weDead && !Partner.partnerDead) ||
         Partner.weVirtualWin))
        return;
...
string UCI::pv(const Position &pos, Depth depth, Value alpha, Value beta)
{
    ...
    for (size_t i = 0; i < multiPV; ++i) {
        ...
        if (Options["Protocol"] == "xboard") {
            ss << d << " " << UCI::value(v) << " " << elapsed / 10 << " " << nodesSearched << " "
               << rootMoves[i].selDepth << " " << nodesSearched * 1000 / elapsed << " " << tbHits
               << "\t";
