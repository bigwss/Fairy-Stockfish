...
std::string StateMachine::highlight(std::string square)
{
    ...
    for (Rank r = pos.max_rank(); r >= RANK_1; --r) {
        for (File f = FILE_A; f <= pos.max_file(); ++f) {
            for (emptyCnt = 0;
                 f <= pos.max_file() && !((promotions | captures | quiets) & make_square(f, r));
                 ++f)
                ++emptyCnt;
            ...
            if (f <= pos.max_file())
                ss << (promotions & make_square(f, r) ? "M"
                       : captures & make_square(f, r) ? "R"
                                                      : "Y");
        }
...
void StateMachine::process_command(std::string token, std::istringstream &is)
{
    if (token == "protover") {
        ...
        sync_cout << "feature setboard=1 usermove=1 time=1 memory=1 smp=1 colors=0 draw=0 "
                  << "highlight=1 name=0 sigint=0 ping=1 myname=\"" << engine_info(false, true)
                  << "\" "
                  << "variants=\"" << vars << "\"" << Options << sync_endl;
        ...
        if (is >> token) {
            if (Threads.main()->ponder) {
                if (token == UCI::square(pos, from_sq(moveList.back())))
                    ...
                    copy_if(moves.begin(), moves.end(), back_inserter(filteredMoves),
                            [&](const Move m) {
                    ...
                    if (filteredMoves.size()) {
                        static PRNG rng(now());
                        ponderMove =
                            filteredMoves.at(rng.rand<unsigned>() % filteredMoves.size());
                    } else
        ...
        if (std::getline(is, token, '[') && std::getline(is, white_holdings, ']') &&
            ...
            if (is >> color && is >> pieceType) {
                ...
                fen.insert(fen.find(']'), 1,
                           toupper(color) == 'B' ? tolower(pieceType) : toupper(pieceType));
            } else {
                std::transform(black_holdings.begin(), black_holdings.end(),
                               black_holdings.begin(), ::tolower);
