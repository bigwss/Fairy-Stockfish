...
extern "C" {
typedef bool (*fun1_t)(LOGICAL_PROCESSOR_RELATIONSHIP, PSYSTEM_LOGICAL_PROCESSOR_INFORMATION_EX,
                       PDWORD);
...
void dbg_print()
{
    ...
    if (means[0])
        cerr << "Total " << means[0] << " Mean " << (double)means[1] / means[0] << endl;
}
...
/// aligned_large_pages_alloc() will return suitably aligned memory, if possible using large
/// pages.
...
static void *aligned_large_pages_alloc_windows(size_t allocSize)
{
    ...
    if (LookupPrivilegeValue(NULL, SE_LOCK_MEMORY_NAME, &luid)) {
        ...
        // succeeds, we still need to query GetLastError() to ensure that the privileges were
        // actually obtained.
        if (AdjustTokenPrivileges(hProcessToken, FALSE, &tp, sizeof(TOKEN_PRIVILEGES), &prevTp,
            ...
            mem = VirtualAlloc(NULL, allocSize, MEM_RESERVE | MEM_COMMIT | MEM_LARGE_PAGES,
                               PAGE_READWRITE);
